...
class BandwidthMonitor(app_manager.RyuApp):
    ...
    def statsReplyHandler(self, ev):
        ...
        edgeSwitch = self.topo.edgeSwitches
        totalDropped = 0
        for stat in body:
        ...
        if name in edgeSwitch.keys():
           # edge Switch
            for stat in body:
                if stat.port_no in self.topo.ports[name]:
                    if self.topo.ports[name][stat.port_no] in self.topo.edgeSwitches[name].neighbors:
